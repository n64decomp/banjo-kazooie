...
void func_80389850(Actor *this, s32 arg1) {
    ...
    for(var_s0 = 2; var_s0 != 0x3C; var_s0++){
        ...
        sp7C[1] = sp94[1];
        sp7C[0] = 0.0f;
        if(func_80256064(sp94, sp7C) <= 1000.0f){
            break;
        }
    }
...
bool func_8038AA2C(ActorMarker *marker, f32 position[0], f32 yaw, f32 dst[3]) {
    ...
    for(i_ptr = begin; i_ptr < end; i_ptr++){
        if( ((local->current_type != YUMBLIE) && (i_ptr->type != YUMBLIE)) 
            ...
            distance = func_80256064(i_ptr->position, position);
            ...
            if( (distance > 300.0f) 
                ...
                if ((closest_piece == NULL) || (distance < func_80256064(position, closest_piece->position))){
                    closest_piece = i_ptr;
                }
            }
        }
