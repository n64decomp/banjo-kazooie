...
void func_802F87B0(struct6s *this){
    ...
    if(vla_size(this->unk1C) >= this->unk20)
        return;
...
void func_802F8A90(struct6s *this, Gfx **gdl, Mtx **mptr, Vtx **vptr){
    struct5s * startPtr = vla_getBegin(this->unk1C);
    struct5s * iPtr;
    struct5s * endPtr = vla_getEnd(this->unk1C);
...
int func_802F8B50(struct6s *this){
    return this->unk22 != 1  &&  (u32)vla_size(this->unk1C) < 1;
}
...
void func_802F8CD0(struct6s * this){
    ...
    for(i = 0; i < vla_size(this->unk1C); i++){
        iPtr;
        iPtr = vla_at(this->unk1C, i);
